app.factory("hotelsParseSrv", function ($q, $log) {
    ...
    function Issues(parseIssues) {
        ...
        this.solvedDate = parseIssues.get("solvedDate") ? moment(parseIssues.get("solvedDate")) : "";
    ...
    function getHotels() {
        ...
        query.find().then(function (results) {
            var hotels = [];
            for (var i = 0; i < results.length; i++) {
                hotels.push(new Hotel(results[i]));
            }
            async.resolve(hotels);
            $log.info('Hotels found', results);
        }, function (error) {
            $log.error("Error while fetching hotels  " + error);
            async.reject(error);
        });
